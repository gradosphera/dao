...
export class Dao implements Contract {
  ...
  async sendProposeInviteAddress(
    ...
    await provider.internal(via, {
      ...
      body: beginCell()
        ...
        .storeRef(
          // cell transaction_info
          beginCell().storeAddress(AddressToInvite).storeUint(ApprovalBlago, 32).storeUint(ProfitBlago, 32).endCell(),
        )
        .endCell(),
    });
  }
  ...
  async sendProposeUpdateAgreementPercent(
    ...
    await provider.internal(via, {
      ...
      body: beginCell()
        ...
        .storeRef(
          // cell transaction_info
          beginCell().storeUint(AgreementPercentNumerator, 32).storeUint(AgreementPercentDenumerator, 32).endCell(),
        )
        .endCell(),
    });
  }
  ...
  async sendProposeTransferBlago(
    ...
    await provider.internal(via, {
      ...
      body: beginCell()
        ...
        .storeRef(
          // cell transaction_info
          beginCell().storeAddress(Recipient).storeUint(ApprovalBlago, 32).storeUint(ProfitBlago, 32).endCell(),
        )
        .endCell(),
    });
  }
  ...
  async sendApprove(provider: ContractProvider, via: Sender, value: bigint, TransactionIndex: number | bigint) {
    await provider.internal(via, {
      value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell().storeUint(DaoOperationCodes.ApproveTransaction, 32).storeUint(TransactionIndex, 32).endCell(),
    });
  }
